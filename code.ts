...
async function measureLineWidths(node: TextNode, wrapWidth: number): Promise<number[][]> {
...
async function createBlockReplacement(node: TextNode): Promise<RectangleNode | FrameNode> {
  const fontSize = getEffectiveFontSize(node);
...
async function replaceTextNode(textNode: TextNode, parent: SceneNode & ChildrenMixin): Promise<void> {
...
(async function main(): Promise<void> {
  ...
  try {
    ...
    if (node.type === "TEXT") {
      ...
      await replaceTextNode(node, parent as SceneNode & ChildrenMixin);
