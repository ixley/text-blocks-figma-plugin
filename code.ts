...
const FALLBACK_FILL: SolidPaint = {
  type: "SOLID",
...
interface PluginSettings {
  opacity: number; // 0–1  (e.g. 0.4 = 40%). Default: 0.4
  cornerRadius: number; // px.  Default: 10
  skipPrefix: string; // Layers starting with this string are skipped. Default: '' (disabled)
}
...
async function loadSettings(): Promise<PluginSettings> {
  ...
  return stored ? { ...DEFAULT_SETTINGS, ...(stored as Partial<PluginSettings>) } : { ...DEFAULT_SETTINGS };
}
...
function getEffectiveFontSize(node: TextNode): number {
  ...
  if (typeof rangeFs === "number") return rangeFs;
  return FALLBACK_FONT_SIZE;
}

function getEffectiveLineHeight(node: TextNode): number {
  const fontSize = getEffectiveFontSize(node);
  const lh = node.lineHeight;
  // figma.mixed is a unique symbol; symbols are not objects
  if (typeof lh !== "object" || lh === null) return fontSize * 1.2;
  if (lh.unit === "AUTO") return fontSize * 1.2;
  if (lh.unit === "PIXELS") return lh.value;
  // PERCENT
  return (fontSize * lh.value) / 100;
}

function getTextFills(node: TextNode): ReadonlyArray<Paint> {
  const fills = node.fills;
  // figma.mixed is a symbol; a symbol is not an array
  if (Array.isArray(fills) && fills.length > 0) {
    return fills as ReadonlyArray<Paint>;
  }
...
function getContentWidth(node: TextNode): number {
  const renderBounds = node.absoluteRenderBounds;
  if (!renderBounds || renderBounds.width <= 0) return node.width;
...
function getClippedVisibleHeight(node: TextNode): number {
  const ownBounds = node.absoluteBoundingBox;
  if (!ownBounds) return node.height;

  let top = ownBounds.y;
  let bottom = ownBounds.y + ownBounds.height;

  for (let current = node.parent; current; current = current.parent) {
    const frame = current as unknown as { clipsContent?: boolean; absoluteBoundingBox?: Rect | null };
...
function getContentXOffset(node: TextNode): number {
  const renderBounds = node.absoluteRenderBounds;
  const bboxBounds = node.absoluteBoundingBox;
  if (!renderBounds || !bboxBounds) return 0;
...
function getContentYOffset(node: TextNode): number {
  const renderBounds = node.absoluteRenderBounds;
  const bboxBounds = node.absoluteBoundingBox;
  if (!renderBounds || !bboxBounds) return 0;
...
function getCounterAxisAlign(node: TextNode): "MIN" | "CENTER" | "MAX" {
...
function getParagraphSpacing(node: TextNode): number {
  const ps = node.paragraphSpacing;
  if (typeof ps === "number") return Math.max(0, ps);
  return 0; // figma.mixed
}

// Returns the first concrete FontName from the text node, falling back to Inter Regular.
function getFirstFontName(node: TextNode): FontName {
  const fn = node.fontName;
  if (typeof fn !== "symbol") return fn as FontName;
  if (node.characters.length > 0) {
    const rangeFn = node.getRangeFontName(0, 1);
    if (typeof rangeFn !== "symbol") return rangeFn as FontName;
  }
...
async function loadFontsForTextNode(node: TextNode): Promise<void> {
  ...
  const seen = new Set<string>([`${primaryFont.family}::${primaryFont.style}`]);
  const toLoad: FontName[] = [primaryFont];
  ...
  const uncached = toLoad.filter((fn) => {
    ...
    if (loadedFonts.has(key)) return false;
    loadedFonts.add(key);
    return true;
  });
...
function simulateWordWrap(text: string, maxWidth: number, temp: TextNode): number[] {
...
async function measureLineWidths(node: TextNode, wrapWidth: number, sharedTemp?: TextNode): Promise<number[][]> {
  await loadFontsForTextNode(node);

  const ownTemp = sharedTemp === undefined;
  const temp = ownTemp ? figma.createText() : sharedTemp;
  if (ownTemp) temp.visible = false;
  ...
  try {
    ...
    if (ownTemp) temp.remove();
  }
}

// ── Shape builders ─────────────────────────────────────────────────────────

function createLineRect(width: number, height: number, fills: ReadonlyArray<Paint>): RectangleNode {
...
async function createBlockReplacement(node: TextNode, sharedTemp?: TextNode): Promise<RectangleNode | FrameNode> {
  const fontSize = getEffectiveFontSize(node);
  ...
  const linesFitInBox =
    isFixedHeight || isEndingTruncated || hasMaxHeight
      ? Math.max(1, Math.floor((node.height + effectiveLH * 1e-4) / effectiveLH))
      : Infinity;
  const clippedVisibleHeight = getClippedVisibleHeight(node);
  const linesFitInClippedAncestor =
    clippedVisibleHeight < node.height - effectiveLH * 1e-4
      ? Math.max(1, Math.floor((clippedVisibleHeight + effectiveLH * 1e-4) / effectiveLH))
      : Infinity;
  const maxLinesCap =
    isEndingTruncated && typeof node.maxLines === "number" && node.maxLines > 0 ? node.maxLines : Infinity;
...
function applyLayoutProps(from: TextNode, to: RectangleNode | FrameNode, xOffset = 0, yOffset = 0): void {
  ...
  const f = from as unknown as { layoutAlign?: string };
  const t = to as unknown as { layoutAlign?: string };
  if (typeof f.layoutAlign === "string" && f.layoutAlign !== "STRETCH") {
    t.layoutAlign = f.layoutAlign as "MIN" | "CENTER" | "MAX" | "INHERIT";
  }
}

// ── Tree traversal ─────────────────────────────────────────────────────────

function hasTextDescendant(node: SceneNode & ChildrenMixin): boolean {
...
function isVisiblyEmpty(node: TextNode): boolean {
  return INVISIBLE_TEXT_RE.test(node.characters);
}

async function replaceTextNode(textNode: TextNode, parent: SceneNode & ChildrenMixin, sharedTemp?: TextNode): Promise<void> {
  ...
  if (isVisiblyEmpty(textNode)) return;

  const children = parent.children as ReadonlyArray<SceneNode>;
  const index = children.indexOf(textNode);
  if (index === -1) return; // Defensive: node not found in parent

  // Read layout sizing BEFORE removal — node properties become unreliable after.
  const nodeAsAL = textNode as unknown as { layoutSizingHorizontal?: string; layoutSizingVertical?: string };
  ...
  const isAutoLayoutParent = "layoutMode" in parent && (parent as FrameNode).layoutMode !== "NONE";
  if (isAutoLayoutParent) {
    const repAsAL = replacement as unknown as { layoutSizingHorizontal?: string; layoutSizingVertical?: string };
...
async function processNode(node: SceneNode, sharedTemp?: TextNode): Promise<void> {
  ...
  for (const child of childSnapshot) {
    ...
    if (settings.skipPrefix && child.name.startsWith(settings.skipPrefix)) continue;
...
(async function main(): Promise<void> {
  ...
  if (figma.command === "settings") {
    ...
    figma.ui.onmessage = async (msg: { type: string; settings?: PluginSettings }) => {
